...
# Downloads and installs Zabbix Agent 2 from scratch on Windows Server 2012 R2+,
# configures PSK encryption, creates a firewall rule, and outputs the PSK
# identity + value needed in the Zabbix frontend.
#
...
# Use a predictable PSK identity for every endpoint.
...
$LogPath = "$env:TEMP\Install-ZabbixAgent2.log"
# ---------------------------------------------------------------
...
function Get-ZabbixAgent2Paths {
    ...
    if (-not $service) {
        throw 'Zabbix Agent 2 service not found after install.'
    }
...
Write-Info "Agent exe:      $($agent.ExePath)"
Write-Info "Config path:    $($agent.ConfigPath)"
...
$configLines += @(
    '',
    '# Managed by Install-ZabbixAgent2-PSK-Action1.ps1',
    "Server=$ZabbixServer",
...
# ---------------------------------------------------------------------------
# Step 5 — Generate random PSK and write the PSK file
# ---------------------------------------------------------------------------
...
$logCheck = Join-Path $agent.AgentDirectory 'zabbix_agent2.log'
if (Test-Path -LiteralPath $logCheck) {
    Get-Content -LiteralPath $logCheck -Tail 10
}
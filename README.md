...
`New-IntuneWinPackage.ps1` downloads Microsoft's Win32 Content Prep Tool from GitHub and packages one or more installer files, or a complete source folder plus its setup file, as `.intunewin` files while printing every output path. `rename-pc.ps1` renames computers during Autopilot enrollment based on serial number and asset tag from a CSV file.
